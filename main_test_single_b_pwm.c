...
void hal_task(void) {
  ...
  if (updated1)
    hal_pwm_update_b(HAL_PWM1, PERIOD_NS, duty1);
  if (updated2)
    hal_pwm_update_b(HAL_PWM2, period2, 0.5f);
  if (updated3)
    hal_pwm_update_b(HAL_PWM3, period3, duty3);
  if (updated4)
    hal_pwm_update_b(HAL_PWM4, period4, duty4);
}
...
void main(void) {
  ...
  hal_pwm_enable_b(HAL_PWM2, (PERIOD_MIN_NS + PERIOD_MAX_NS) / 2, 0.5f);
  hal_pwm_enable_b(HAL_PWM3, (PERIOD_MIN_NS + PERIOD_MAX_NS) / 2, 0.5f);
  hal_pwm_enable_b(HAL_PWM4, PERIOD_MIN_NS, DUTY_MIN);
