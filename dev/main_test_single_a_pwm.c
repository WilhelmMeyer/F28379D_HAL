...
#define TRIGGER_PIN_CONFIG GPIO_16_GPIO16

#define TASK_PERIOD_NS 20000

#define PERIOD_NS 20000 // 50kHz fixo
#define PERIOD_MIN_NS 10000
#define PERIOD_MAX_NS 50000
#define PERIOD_STEP_NS 50
#define PERIOD_STEP_FAST_NS 5000

#define DUTY_MIN 0.2f
...
void hal_task(void) {
  ...
  // PWM4 — duty e período variam bruscamente
  static float duty4 = DUTY_MIN;
  ...
  if (++counter4 >= UPDATE_FAST) {
    counter4 = 0;
    duty4 = (duty4 == DUTY_MIN) ? DUTY_MAX : DUTY_MIN;
  ...
  if (updated1)
    hal_pwm_update_a(HAL_PWM1, PERIOD_NS, duty1);
  if (updated2)
    hal_pwm_update_a(HAL_PWM2, period2, 0.5f);
  if (updated3)
    hal_pwm_update_a(HAL_PWM3, period3, duty3);
  if (updated4)
    hal_pwm_update_a(HAL_PWM4, period4, duty4);
}

void main(void) {
  hal_init();

  hal_pwm_enable_a(HAL_PWM1, PERIOD_NS, 0.5f);
  hal_pwm_enable_a(HAL_PWM2, (PERIOD_MIN_NS + PERIOD_MAX_NS) / 2, 0.5f);
  hal_pwm_enable_a(HAL_PWM3, (PERIOD_MIN_NS + PERIOD_MAX_NS) / 2, 0.5f);
  hal_pwm_enable_a(HAL_PWM4, PERIOD_MIN_NS, DUTY_MIN);

  GPIO_setPinConfig(TRIGGER_PIN_CONFIG);
